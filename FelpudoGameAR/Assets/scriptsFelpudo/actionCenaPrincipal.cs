...
public class actionCenaPrincipal : MonoBehaviour {
	...
	void Start () {
		...
		if(Application.loadedLevelName == "cenaFelpudo")
		 {cartaoDetectado = true;}else{cartaoDetectado = false;}
		...
		{ 
			if(!acabou)
			{
					...
					
				{
					...
					passarinho.rigidbody.useGravity = true;
					comecou = true;
					scrollSpeed = -0.5f;
					voaBird();
					textoMensagem.text = "";
	...
	void criaCerca()
	 {
		if(comecou && cartaoDetectado){
			...
			novoObjeto.rigidbody.AddForce (new Vector3(0,0,velocidadeObjeto), ForceMode.Force);
		}
	}
	...
	void criaArbustoNuvemPedra()
	 {  
		if(comecou && cartaoDetectado)
		{
			...
			objetoX.rigidbody.AddForce (new Vector3(0,0,velocidadeObjeto), ForceMode.Force); 
	...
	{ 
		passarinho.rigidbody.velocity = new Vector3(0,0,0);
		passarinho.rigidbody.AddForce (new Vector3(0,200,0), ForceMode.Force);
		criaPeninhas();
		audio.PlayOneShot(somVoa, 0.5F);
	}  


	void LateUpdate () 
	{
		 velocidade = passarinho.rigidbody.velocity.y; 
	...
	public void fimDeJogo()
	 {
		 if(!acabou)
		 {
			...
			audio.PlayOneShot(somHit, 0.35F);
		 }
	 }
	...
	{ 
		...
		audio.PlayOneShot(somFinal, 0.25F);
	 }
	...
	{ 
		score++;
		 textoScore.text = score.ToString();
		audio.PlayOneShot(somScore, 0.55F);
	 }
	...
	public void criaPeninhas()
	{
		...
		{ 
			...
			novaPeninha.rigidbody.AddForce (new Vector3(dx,dy,dz), ForceMode.Force);
			novaPeninha.rigidbody.AddTorque(new Vector3(rx,ry,rz));
		} 
	}	
